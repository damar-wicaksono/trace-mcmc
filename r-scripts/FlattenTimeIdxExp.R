...
#'
#' The function can also be used for other time indices at different location 
#' given as list
#'
...
#' @return
FlattenTimeIdxExp <- function(time_idx_list, n_time, excl_idx=NULL)
{
    time_idx_vec <- c()

    for (i in 1:length(time_idx_list))
    {
        incl_idx <- setdiff(1:length(time_idx_list[[i]]), excl_idx)
        time_idx_vec <- c(time_idx_vec, 
                          (time_idx_list[[i]][incl_idx] - length(excl_idx)) + 
